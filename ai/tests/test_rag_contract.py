...
from app.rag.retriever import _retrieve_lexical, lexical_score, retrieve_sources
...
class RagContractTest(unittest.TestCase):
    ...
    def test_service_returns_answer_with_citations_and_metadata(self):
                ...
                SimpleNamespace(
                    message=SimpleNamespace(
                        content=(
                            ...
                            '"groundedEvidence":[{"claim":"The source discusses semantic search.","sourceIds":["publication:1"]}],'
        ...
        self.assertEqual(response.answer.groundedEvidence[0].sourceIds, ["publication:1"])
    ...
    def test_service_rejects_invalid_grounded_evidence_source_ids(self):
        source = RagSource(
            id=1,
            type="publication",
            title="Semantic Search for Biomedical Texts",
                ...
                SimpleNamespace(
                    message=SimpleNamespace(
                        content=(
                            '{"executiveSummary":"Semantic search is represented.",'
                            '"evidence":["The source discusses semantic search."],'
                            '"groundedEvidence":[{"claim":"The source discusses semantic search.","sourceIds":["publication:999"]}],'
                            '"limitations":[],"suggestedReadings":[]}'
                        )
                    )
                )
            ],
    ...
    def test_retriever_uses_lexical_fallback_when_semantic_search_fails(self):
        source = RagSource(
            id=1,
            type="publication",
            title="Semantic Search for Biomedical Texts",
        ...
        settings = SimpleNamespace(rag_max_sources=8)

        with (
            patch("app.rag.retriever.get_settings", return_value=settings),
            patch("app.rag.retriever._retrieve_semantic", side_effect=RuntimeError("missing table")),
            patch("app.rag.retriever._retrieve_lexical", return_value=([source], [])),
        ):
            sources, warnings = retrieve_sources("semantic search", ["publications"])

        self.assertEqual(sources, [source])
        self.assertIn("Semantic entity search unavailable; lexical fallback was used.", warnings)

    def test_lexical_retriever_excludes_prompt_injection_sources(self):
        docs = [
            {
                "id": 1,
                "title": "Semantic Search for Biomedical Texts",
                "abstract": "Semantic search improves retrieval.",
        ...
        settings = SimpleNamespace(rag_min_source_score=0.0)

        with (
            patch("app.rag.retriever.get_settings", return_value=settings),
            patch("app.rag.retriever.payload_api.find_all", return_value=docs),
        ):
            sources, warnings = _retrieve_lexical("semantic search", ["publications"], 8)

        self.assertEqual([source.id for source in sources], [1])
        self.assertIn("Potential prompt injection pattern found in publications/2; source excluded.", warnings)
